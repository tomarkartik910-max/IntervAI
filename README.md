...
`VITE_FIREBASE_APIKEY`	`client/.env`	Firebase client configuration
`VITE_RAZORPAY_KEY_ID`	`client/.env`	Razorpay checkout key
`PORT`	`server/.env`	Backend server port
`MONGODB_URL`	`server/.env`	MongoDB connection string
`JWT_SECRET`	`server/.env`	JWT signing secret
`OPENROUTER_API_KEY`	`server/.env`	OpenRouter API authentication
`RAZORPAY_KEY_ID`	`server/.env`	Razorpay server key
`RAZORPAY_KEY_SECRET`	`server/.env`	Razorpay server secret
